...
#include <Ticker.h>        // Built-in ESP32 timer for LED blinking during portal mode

// ============================================================
//  ESP32 + W5500 — Duke Side (Lift 3 & 4) + Temperature
//  Ethernet = primary link. WiFi = automatic fallback only.
...
// ║  All parameters you may need to change are listed here.      ║
// ║  Edit ONLY this section when deploying to a different node.  ║
// ╚══════════════════════════════════════════════════════════════╝
...
const int LED_WIFI  = 25;   // optional: lit while running on WiFi fallback

// ---------- Dedicated WiFi setup button ----------
// Separate from the lift buttons on purpose, per requirements.
...
const unsigned long CONNECTION_PING_INTERVAL_MS = 30000;  // compare both paths every 30 seconds

// ---------- Boot race timings (keep boot fast) ----------
const unsigned long BOOT_ETH_LINK_MS      = 1500;  // max wait for physical Ethernet link
const unsigned long BOOT_DHCP_TIMEOUT_MS  = 4000;  // max wait for DHCP at boot
const unsigned long BOOT_DHCP_RESPONSE_MS = 1000;  // DHCP single-response timeout
const unsigned long BOOT_WIFI_CONNECT_MS  = 5000;  // max wait for WiFi at boot
...
//  EthernetClient and WiFiClient both implement Arduino's Client
//  interface, so this works transparently for postLineProtocol().
// ============================================================
Client* getActiveClient() {
  return (connMode == MODE_WIFI) ? (Client*)&wifiClient : (Client*)&ethClient;
}

// Compare TCP connection latency to the InfluxDB endpoint without interrupting
// the active transport. This is used as a practical reachability/latency probe.
...
void selectBestConnection(bool forceProbe) {
  bool ethAvailable = (Ethernet.linkStatus() == LinkON);
  bool wifiAvailable = (WiFi.status() == WL_CONNECTED);
  if (!ethAvailable || !wifiAvailable) return;
...
// ============================================================
//  postLineProtocol()
//  Shared HTTP POST helper, now transport-agnostic.
// ============================================================
...
// ============================================================
//  bootCheckWifiButtonHeld()  [boot-time only]
//  Used inside boot wait loops. Returns true if the portal ran.
// ============================================================
bool bootCheckWifiButtonHeld() {
  bool pressed = !digitalRead(WIFI_SETUP_BUTTON_PIN);
  if (!pressed) { wifiButtonPressStart = 0; return false; }
  if (wifiButtonPressStart == 0) { wifiButtonPressStart = millis(); return false; }
...
WiFiManagerParameter portalInfo(
  "<p><b>Lift Alarm Node Duke - WiFi Setup</b><br>"
  "Enter the WiFi network this node should use when Ethernet is unavailable.</p>"
);

void onWifiConfigSaved() {
  Serial.println(F("[PORTAL] WiFi credentials saved"));
}
...
// ============================================================
//  startWifiConfigPortal()
//  Blocking. Runs when setup button is held (boot or runtime).
//  Blinks the WiFi LED while in portal mode to show config state.
// ============================================================
void startWifiConfigPortal() {
  // Start blinking WiFi LED rapidly (250ms on / 250ms off) to indicate config mode
  wifiLedTicker.attach_ms(250, tickWifiLed);
  Serial.println(F("[PORTAL] Starting WiFi config portal (LED blinking)..."));

  WiFiManager wm;
  wm.setConfigPortalTimeout(180);
  wm.setConnectTimeout(20);
  wm.setBreakAfterConfig(true);
  wm.setTitle("Lift Alarm Node - Duke");
  wm.setClass("invert");
  ...
  // Stop blinking when portal exits
  wifiLedTicker.detach();

  if (connected) {
    Serial.println(F("[PORTAL] Connected with new credentials."));
    connMode = MODE_WIFI;
    digitalWrite(LED_WIFI, HIGH);
  } else {
    Serial.println(F("[PORTAL] Portal timed out or exited without saving."));
    digitalWrite(LED_WIFI, LOW);
  }
}
...
//    1. Wait up to BOOT_ETH_LINK_MS for Ethernet physical link.
...
//    4. If neither: hand off to loop() to keep retrying.
...
void bootConnect() {
  ...
  while (!linkUp && millis() - t < BOOT_ETH_LINK_MS) {
  ...
  if (linkUp) {
    ...
    // ---- Stage 2b: DHCP timed out - fall back to static IP ----
  ...
  Serial.println(F("[BOOT] No connection in boot window - loop() will keep retrying."));
  connMode = MODE_ETHERNET;
  ethDownSince    = millis() - LINK_DOWN_GRACE_MS + 1;
  lastWifiAttempt = 0;
}
